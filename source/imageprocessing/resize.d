module imageprocessing.resize;

import imageformats;

// Up or downscales an RGBA image
...
private ulong[] scaleImage(int targetWidth, int targetHeight, in ulong[] source,
...
private ulong interpolate2D(float targetX, float targetY, in ulong[] source,
		int sourceWidth, int sourceHeight)
{
	import std.math;
...
private ulong get(in ulong[] source, int sourceWidth, int sourceHeight, int x, int y)
{
	if (x >= sourceWidth)
		x = sourceWidth - 1;
	if (y >= sourceHeight)
...
private ulong interpolate(ulong a, ulong b, float amount)
{
	return lerp(a, b, amount);
}

private ulong lerp(ulong a, ulong b, float amount)
{
...
private ubyte[4] interpolate2D(float targetX, float targetY, in IFImage source,
		int sourceWidth, int sourceHeight)
{
	import std.math;
...
private ubyte[4] get(in IFImage source, int sourceWidth, int sourceHeight, int x, int y)
{
	if (x >= sourceWidth)
		x = sourceWidth - 1;
	if (y >= sourceHeight)
...
private ubyte[4] interpolate(ubyte[4] a, ubyte[4] b, float amount)
{
	return lerp(a, b, amount);
}

private ubyte[4] lerp(ubyte[4] a, ubyte[4] b, float amount)
{
	float iAmount = 1 - amount;
	ubyte aa = a[3];
	ubyte ba = b[3];
